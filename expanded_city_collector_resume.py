#!/usr/bin/env python3
"""
Expanded City Collector with Resume Capability
Collects all cities within the Dallas-Denison radius with ability to resume interrupted collections
"""
...
class ResumableExpandedCityCollector:
    def __init__(self, api_key: str = None):
        """
        Initialize the Resumable Expanded City Collector
    ...
    def load_existing_data(self) -> Tuple[pd.DataFrame, Set[Tuple[str, int]]]:
        """Load existing data and return DataFrame plus set of (city, year) combinations already collected"""
        existing_files = list(Path('.').glob('north_texas_expanded_demographics_*.csv'))
        
        if existing_files:
            # Load the most recent file
            latest_file = max(existing_files, key=lambda x: x.stat().st_mtime)
            print(f"📂 Found existing data file: {latest_file}")
            
            try:
                df = pd.read_csv(latest_file)
                print(f"✅ Loaded {len(df)} existing records")
                
                # Create set of (city, year) combinations already collected
                existing_combinations = set()
                if 'city' in df.columns and 'year' in df.columns:
                    for _, row in df.iterrows():
                        city = row['city'] if 'city' in df.columns else row['name'].replace(', Texas', '')
                        year = int(row['year'])
                        existing_combinations.add((city, year))
                elif 'name' in df.columns and 'year' in df.columns:
                    for _, row in df.iterrows():
                        city = row['name'].replace(', Texas', '')
                        year = int(row['year'])
                        existing_combinations.add((city, year))
                
                print(f"📊 Found data for {len(set(combo[0] for combo in existing_combinations))} cities")
                print(f"📅 Years covered: {sorted(set(combo[1] for combo in existing_combinations))}")
    ...
    def get_city_coordinates(self, city_name: str) -> Tuple[float, float]:
        """Get approximate coordinates for a city (expanded mapping)"""
        ...
        city_coords = {
            ...
            'Fate city': (32.9373, -96.3869),  # Added Fate city coordinates
    ...
    def collect_expanded_data_resumable(self, start_year: int = 2009, end_year: int = 2022) -> pd.DataFrame:
        """Collect demographic data for all cities in the expanded radius with resume capability"""
        print("🔄 Starting resumable expanded data collection for Dallas-Denison radius...")
        
        # Load existing data
        existing_df, existing_combinations = self.load_existing_data()
        ...
        # Convert existing DataFrame to list of dictionaries for appending
        all_data = existing_df.to_dict('records') if not existing_df.empty else []
        
        # Calculate total work and what's already done
        ...
        for place in filtered_places:
            ...
            city_clean = place_name.replace(', Texas', '')
            
            print(f"\n🏙️ Processing {place_name} (pop: {place['population']:,}, distance: {distance:.1f} mi)...")
            ...
            for year in range(start_year, end_year + 1):
                ...
                # Check if we already have this city-year combination
                if (city_clean, year) in existing_combinations:
                    skipped_requests += 1
                    city_skipped_data += 1
                    continue
                ...
                if new_requests % 10 == 0:  # Progress update every 10 new requests
                    progress_pct = ((skipped_requests + new_requests) / total_requests) * 100
                    print(f"📊 Progress: {progress_pct:.1f}% ({new_requests} new, {skipped_requests} skipped)")
                ...
                if data:
                    ...
                    # Add to existing combinations to avoid duplicates in this session
                    existing_combinations.add((city_clean, year))
                
                time.sleep(0.05)  # Rate limiting
            
            if city_new_data > 0:
                print(f"  ✅ Added {city_new_data} new records (skipped {city_skipped_data} existing)")
            else:
                print(f"  ⏭️ All data already exists (skipped {city_skipped_data} records)")
        ...
        if not df.empty:
            ...
            print(f"\n📈 Collection Summary:")
            print(f"  • New records collected: {new_requests}")
            print(f"  • Existing records skipped: {skipped_requests}")
    ...
    def estimate_county(self, city_name: str) -> str:
        ...
        county_map = {
            ...
            'Fate': 'Rockwall', 'Royse City': 'Rockwall', 'Terrell': 'Kaufman',
            'Forney': 'Kaufman', 'Rockwall': 'Rockwall', 'Wylie': 'Collin'
        }
...
def main():
    """Main function to collect expanded data with resume capability"""
    print("🌟 RESUMABLE EXPANDED NORTH TEXAS DEMOGRAPHIC COLLECTION")
    print("=" * 70)
    ...
    collector = ResumableExpandedCityCollector(api_key=api_key)
    
    # Collect data with resume capability
    print("\nStarting resumable expanded data collection...")
    df = collector.collect_expanded_data_resumable(start_year=2009, end_year=2022)
    ...
    if not df.empty:
        ...
        # Display summary
        print(f"\n🎉 RESUMABLE DATA COLLECTION COMPLETE!")
        print(f"=" * 50)
        ...
        for i, (_, row) in enumerate(top_cities.iterrows(), 1):
            distance = row.get('distance_from_dallas', 0)
            distance_str = f" ({distance:.1f} mi)" if distance > 0 else ""
            print(f"  {i:2d}. {row['city']:<25} {row['total_population']:>8,}{distance_str}")
